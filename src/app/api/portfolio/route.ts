...
// Define a type for incoming holding objects
type HoldingInput = {
  symbol: string;
  name: string;
  quantity: number;
...
export async function POST(request: Request) {
  try {
    ...
    if (holdings && Array.isArray(holdings) && holdings.length > 0) {
      for (const holding of holdings as HoldingInput[]) {
    ...
    if (holdings && holdings.length > 0) {
      const typedHoldings = holdings as HoldingInput[];

      // Fetch prices for holdings
      const symbols = typedHoldings.map((h) => h.symbol);
      const prices = await fetchBatchPrices(symbols);

      // Calculate values
      const holdingsWithValues = typedHoldings.map((h) => ({
        ...h,
